...
#返回变形后的帧，掩码、深度和光流
    ...
    def compute_transformed_points_original(self, depth1: torch.Tensor, transformation1: torch.Tensor, transformation2: torch.Tensor,
    ...
    def compute_transformed_points(self, depth1: torch.Tensor, transformation1: torch.Tensor, transformation2: torch.Tensor,
        ...
        # 将所有输入张量转换为 Float 类型
        depth1 = depth1.float()
        transformation1 = transformation1.float()
        transformation2 = transformation2.float()
        intrinsic1 = intrinsic1.float()
        if intrinsic2 is not None:
            intrinsic2 = intrinsic2.float()
        ...
        x1d = torch.arange(0, w)[None]
        y1d = torch.arange(0, h)[:, None]
        x2d = x1d.repeat([h, 1]).to(depth1).float()  # 转换为 Float 类型
        y2d = y1d.repeat([1, w]).to(depth1).float()  # 转换为 Float 类型
        ones_2d = torch.ones(size=(h, w)).to(depth1).float()  # 转换为 Float 类型
        ones_4d = ones_2d[None, :, :, None, None].repeat([b, 1, 1, 1, 1])  # (b, h, w, 1, 1)
        pos_vectors_homo = torch.stack([x2d, y2d, ones_2d], dim=2)[None, :, :, :, None]  # (1, h, w, 3, 1)

        intrinsic1_inv = torch.linalg.inv(intrinsic1).float()  # 转换为 Float 类型
        intrinsic1_inv_4d = intrinsic1_inv[:, None, None]  # (b, 1, 1, 3, 3)
        intrinsic2_4d = intrinsic2[:, None, None].float()  # 转换为 Float 类型
        depth_4d = depth1[:, 0][:, :, :, None, None]  # (b, h, w, 1, 1)
        trans_4d = transformation[:, None, None]  # (b, 1, 1, 4, 4)
